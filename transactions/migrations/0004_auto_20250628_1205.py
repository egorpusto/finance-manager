...
def remove_duplicate_categories(apps, schema_editor):
    ...
    users_with_categories = Category.objects.values_list("user", flat=True).distinct()

    for user_id in users_with_categories:
        user_categories = Category.objects.filter(user_id=user_id)

        unique_names = user_categories.values_list("name", flat=True).distinct()
...
class Migration(migrations.Migration):
    dependencies = [
        ...
        (
            "transactions",
            "0003_alter_category_options_alter_transaction_options_and_more",
        ),
    ]
